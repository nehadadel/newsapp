    ...
    private static List<News> extractFeatureFromJson(String newsJSON) {
        ...
        try {
            ...
            for (int i = 0; i < resultsArray.length(); i++) {
                ...
                JSONObject tagsname = tags.getJSONObject(0);
                String author = tagsname.getString("webTitle");

                // Create a new {@link News} object with the title and url from the JSON response.
                News news = new News(webTitle, sectionName, webPublicationDate, author, webUrl);
